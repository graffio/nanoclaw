...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // .env shadowing: the container entrypoint uses `mount --bind /dev/null`
    // inside the container to shadow .env. No host-side mount needed (Apple
    // Container only supports directory mounts, not file mounts).
    ...
    // Main gets writable access to global directory (for shared knowledge bases)
  ...
  // groups. Always re-synced to pick up upstream changes (like skills).
