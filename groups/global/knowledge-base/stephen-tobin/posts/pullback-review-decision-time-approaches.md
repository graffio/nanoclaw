---
title: "Pullback Review: Decision Time Approaches"
date: 2025-11-05
url: https://stephentobin.substack.com/p/pullback-review-decision-time-approaches
audience: only_paid
post_type: thematic
summary: "You always need a plan"
---

# Pullback Review: Decision Time Approaches

*You always need a plan*

Without a plan, you will react and make snap decisions under the pressure of mounting losses, inevitably leading to mistakes. Without a plan, we become gamblers and not traders.

We need a plan because markets are pulling back, and it brings doubt and fear. Even though we all know we are investing in risky securities, we tend to overlook the downside risks, despite logically knowing that they exist and that the pain of a $1,000 loss is felt more than the pleasure of a $1,000 win.

We need a plan for the situation we are in.

## The Situation

We have a portfolio filled with small-cap, early-stage companies that move farther and faster than the indices. The portfolio has been highly profitable, achieving three consecutive years of triple-digit percentage gains.

Perhaps things are changing, and this week's 5% loss is a sign of changing times. Multiple outlets are predicting a market correction, and if one arrives, our portfolio will suffer, and we can do without that.

The Asian markets followed the US ones lower overnight with significant falls across all major indices. European stocks have opened mixed, with Germany and France down, but the UK up. Futures are again pointing lower for the US amid volatile trading. The Asian index futures are generally pointing a little higher.

So far, the total moves lower do not indicate a major sell-off, but of course, things could just be starting.

[

![A table with numbers and percentages

AI-generated content may be incorrect.](https://substack-post-media.s3.amazonaws.com/public/images/1412c0cf-2dff-4b68-89a5-1da4b43dab6b_1054x322.png)



](https://substackcdn.com/image/fetch/$s_!SUPM!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F1412c0cf-2dff-4b68-89a5-1da4b43dab6b_1054x322.png)

We are a long way from a bear market, and as a result, there is no need for a wholesale move to cash.

If we do see a more sustained move lower, I will once again liquidate our riskier positions first and exit the largest positions, to protect capital and buy again later in the month, or even next month if the drop is prolonged. Drops are typically short and sharp, but even so, as an active manager, I do not want to suffer a large drawdown.

I have been conscientious in managing my position size, ensuring the account maintains a decent cash reserve. This allows us to weather any drawdown and have ample funds to invest later.

The account balances are as follows (European markets are open, US and Asia closed at time of snapshot)

[

![A table with numbers and a few dollar bills

AI-generated content may be incorrect.](https://substack-post-media.s3.amazonaws.com/public/images/259d9a0c-dccd-4f36-be38-b2a0c2b62e8c_660x462.png)



](https://substackcdn.com/image/fetch/$s_!Q_-V!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F259d9a0c-dccd-4f36-be38-b2a0c2b62e8c_660x462.png)

We have lost approximately half of the profit made last month, and the account is experiencing a drawdown of more than 4%. Some drawdown was almost inevitable after seven consecutive monthly wins. This is not enough to cause any panic, but we do need a plan to avoid any panic, as that will cost us more money than any pullback.

We are in a time of heightened volatility and geopolitical tension, plus the ongoing US government shutdown will begin to impact economic activity. Often, these things become a self-fulfilling prophecy; many pundits predict a pullback, prompting people to sell their shares. It is this selling action that ultimately causes the pullback.

In these situations, you need a plan. What stocks are you going to hold no matter what, which are on the chopping block, and are there any you think it might just be better to offload regardless of the situation?

**Disclaimer:** I’m not a financial advisor and don’t offer investment advice. This newsletter covers **my high-risk trading in small-cap emerging stocks;** past performance doesn’t guarantee future returns. Make independent investment decisions based on your own research and risk tolerance; you are solely responsible for outcomes.

**Trade Alert:** Preparing to exit Airo Holdings and Volatus Aerospace

I will place stop losses at -30% for Volatus (TAKOF) or close at market open if it drops below this figure and -40% for Airo (AIRO) with the same plan.

## Designing a Plan of Action

I have categorized my current holdings by sector (where possible) to compare the performance of individual stocks with the sector I follow. I am looking for anomalies, a stock performing worse than its sector, indicating something may be amiss with that particular stock.

I have highlighted the figures that give me concern in red. The top blue line gives the sector information: the number of companies and their average return. Following these are the individual positions, total holding, total return, and return over the last seven days. Total holdings are in the currency of origin so CWR is in GBP, GRA, ROOF are in CAD and EOS, DRO are in AUS.

[

![Image 3](https://substack-post-media.s3.amazonaws.com/public/images/45dff5d9-3a55-4403-86a1-1762caf3dfc9_1053x540.png)



](https://substackcdn.com/image/fetch/$s_!Tp_f!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F45dff5d9-3a55-4403-86a1-1762caf3dfc9_1053x540.png)

Most of our holdings are moving roughly in line with their sector, which is a sign of a general market pullback; all ships rise with the tide.

ELVA and CWR are the biggest holdings and I will look to close them quickly if the markets do turn lower with greater momentum. For the other holdings highlighted as a concern my plan of action is.

[

![A screenshot of a white table with blue text

AI-generated content may be incorrect.](https://substack-post-media.s3.amazonaws.com/public/images/5e1235cf-5f78-4b5b-87af-c0b47b9bda3d_304x356.png)



](https://substackcdn.com/image/fetch/$s_!rZuT!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F5e1235cf-5f78-4b5b-87af-c0b47b9bda3d_304x356.png)

**Hold** means I currently have no intention of selling.

**Monitor** means precisely that, I am going to keep these under watch and exit if any sustained selling arrives.

**Close if** means I would like to keep these stocks if possible, but they are approaching a -30% loss, and if they reach that point, the decision will likely be to exit. I will send an additional trade alert if I decide to take action on these two.

**Close:** For AIRO, I will enforce a 40% loss rule, and for TAKOF, a 30% loss rule. I will not allow the stocks to drop below this figure and will watch the market open taking the opportunity to add a stop loss order if I get the chance.

---

*Source: [Strategic Wave Trading](https://stephentobin.substack.com/p/pullback-review-decision-time-approaches)*
